...
import { loadCheckout } from "../lib (LIBRARY FOLDER)/stripe";
...
function Plans({products}:Props) {
  ...
  return (
    ...
    <main className="mx-auto pt-28 max-w-5xl px-5 pb-12
     transition-all md:px-10">
            ...
            <div className="flex w-full items-center justify-center self-end md:w-3/5">
                ...
                {products.map((product) => (//mapped through every single product
                    ...
                    onClick={()=> setSelectedPlan(product)}>
                      {/* now i can click opacity changes on plan i selected */}
                      {product.name}</div>
                ))}
            </div>
